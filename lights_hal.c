...
static int set_light_backlight (struct light_device_t *dev, struct light_state_t const *state) {
	...
	/** 
	 *	brightness: 0-255
	 *	LCD_BACKLIGHT_FILE supports 0-127 from drivers/input/touchscreen/tiny4412_1wire_host.c +239 line
	 */
...
static int set_light_battery (struct light_device_t *dev, struct light_state_t const* state) {
	...
	ALOGV("%s flashMode = %d flashOnMS = %d flashOffMS = %d color = 0x%08x", __func__, state->flashMode, 
										state->flashOnMS, state->flashOffMS, state->color);
